...
const urls = [
    ...
    , {
        ...
        method: 'POST',
        getData: (request, id) => {
            let found = storage.data.find(item => request.body.firstName === item.firstName && 
                request.body.lastName === item.lastName &&
                request.body.email === item.email);
                if(found) {
                    return throwError("Duplicate Contact");
                } 
